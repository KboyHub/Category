...
- (void)setBadgeValue:(int)value
{
    if(value != 0 || self.displayWhenZero) {
        CGSize numberSize = [[NSString stringWithFormat:@"%d", (int)value] sizeWithFont:self.font];
        ...
        if(self.placement == kBadgePlacementUpperBest) {
            CGPoint rightMostInWindow = [self.superview convertPoint:CGPointMake(superviewFrame.origin.x + superviewFrame.size.width + (diameter*0.5), -(diameter*0.5)) fromView:nil];
        ...
        self.center = (self.placement == kBadgePlacementUpperLeft) ? CGPointMake(0, 0) : CGPointMake(superviewFrame.size.width*0.8, superviewFrame.size.height*0.2);
    } else {
...
//        CGContextFillEllipseInRect(context, CGRectInset(rect, self.outlineWidth + 1, self.outlineWidth + 1));
        ...
        [[NSString stringWithFormat:@"%d", self.badgeValue] drawInRect:CGRectMake(self.outlineWidth + 3, (rect.size.height*0.5) - (numberSize.height*0.5), rect.size.width - (self.outlineWidth*2) - 6, numberSize.height) withFont:self.font lineBreakMode:NSLineBreakByClipping alignment:NSTextAlignmentCenter];
    }
}
